...
async function fetchData() {
    try {
        ...
        if (json.status === 'success' && json.data) {
            db.articles = json.data.filter(art => art.category?.toLowerCase() === 'science');
        } else {
            db.articles = [];
        }
        renderAll();
...
async function addArticle() {
    ...
    await syncArticle({ title, content, category: "Science", link });
...
function renderAll() {
    ...
    if (db.articles.length === 0) {
        container.innerHTML = `<p class="no-data-msg">No secure science records found in the network.</p>`;
        return;
    }
